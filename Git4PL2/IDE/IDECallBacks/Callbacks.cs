...
namespace Git4PL2.PLSqlDev.IDECallBacks
{
    ...
    public static class Callbacks2
    {
        ...
        public static T GetDbObject<T>() where T : DbObject
        {
            ...
            Seri.Log.Here().Debug("Запрос объекта БД");
        ...
        public static string GetDatabaseConnection()
        {
            ...
            Seri.Log.Here().Info("Cоединение БД: {0}", Database);
            return Database;
        }
        ...
        {
            Seri.Log.Here().Verbose("Проверяем наличие соединения");
            IDE_Connected IDE_ConnectedCallback = GetDelegate<IDE_Connected>();
            if (IDE_ConnectedCallback == null)
            {
                Seri.Log.Here().Verbose("Делегат не установлен. Выходим.");
                return false;
            }
            bool ans = IDE_ConnectedCallback?.Invoke() ?? false;
            Seri.Log.Here().Verbose("Соединение {0}установлено", ans? "" : "НЕ ");
            return ans;
        }
        ...
        public static int GetWindowType()
        {
            ...
            Seri.Log.Here().Verbose("Тип окна PL/SQL Developer - {0}", ans);
            return ans;
        }
        ...
        {
            Seri.Log.Here().Debug("GoToLine begin: LineNum={0}, BasePos={1}", LineNum, BasePos);
